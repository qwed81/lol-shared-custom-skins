...
namespace ClientStore.Network
{
    internal class FileRetriever
    {
        ...
        public async Task RetrieveFileIfAbsentAsync(Guid sessionId, Guid userId, FileDescriptor fd, FileType fileType)
        {
            ...
            using (TcpClient client = new TcpClient())
            {
                ...
                while (client.Connected && totalAmtRead < size)
                {
        ...
        {
            var request = new FileRequestMetadata(sessionId, Guid.NewGuid(), fd);
